...
def test_log_and_progression(client):
    ...
    r = client.post("/api/sets", json={
        ...
        "weight_kg": 45.4, "reps": 12, "rpe": 8, "notes": "felt strong"})
...
def test_edit_and_delete(client):
    ...
    eid = client.post("/api/sets", json={
        ...
        "weight_kg": 1000, "reps": 10}).json()["id"]  # fat-finger
    fixed = client.patch(f"/api/entries/{eid}", json={"weight_kg": 24}).json()
    assert fixed["weight_kg"] == 24
...
def test_volume_and_metrics(client):
    ...
    client.post("/api/metrics", json={"metric": "bicep_l", "value": 35.5, "unit": "cm"})
    series = client.get("/api/metrics/bicep_l").json()
    assert series and series[-1]["value"] == 35.5
